...
class Example(object):
  ...
  def __init__(self, article, abstract_sentences, rel_scores, vocab, hps):
    ...
    if len(article_words) > hps.max_enc_steps:
      ...
      rel_scores = rel_scores[:hps.max_enc_steps]
...
class Batch(object):
  ...
  def init_encoder_seq(self, example_list, hps):
    ...
    self.scores = np.zeros((hps.batch_size, max_enc_seq_len), dtype=np.float32)
...
class Batcher(object):
  ...
  def fill_example_queue(self):
    ...
    while True:
      try:
        (article, abstract, rel_scores) = input_gen.next() # read the next example from file. article and abstract are both strings.
      ...
      example = Example(article, abstract_sentences, rel_scores, self._vocab, self._hps) # Process into an Example.
  ...
  def text_generator(self, example_generator):
    ...
    while True:
      ...
      try:
        ...
        relevancy_score = np.array([np.float32(s) for s in str(relevancy_score)[3:-2].split(',')], dtype=np.float32)
        print 'read!!!'
      except ValueError:
